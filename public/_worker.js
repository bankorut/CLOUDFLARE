        ...
        try {
            ...
            if (path === "/api/ai/chat" && method === "POST") {
                ...
                const aiRes = await env.AI.run("@cf/meta/llama-3-8b-instruct", {
                    messages: [
                        { role: "system", content: "Kamu adalah asisten AI ramah di aplikasi Cloudflare Social App." },
                        { role: "user", content: prompt }
                    ]
                });
                const textReply = aiRes.response || aiRes.description || (typeof aiRes === 'string' ? aiRes : "Halo! Ada yang bisa saya bantu?");
